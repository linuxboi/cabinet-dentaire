...
import { FaBars, FaTimes, FaPhoneAlt, FaTooth, FaChevronDown, FaWhatsapp } from 'react-icons/fa';
...
const Header = () => {
  ...
  const navLinks = [
    ...
    { path: '/equipe', label: 'Équipe', hasDropdown: false },
    { path: '/services', label: 'Soins', hasDropdown: true },
    { path: '/avant-apres', label: 'Avant/Après', hasDropdown: false },    { path: '/contact', label: 'Contact', hasDropdown: false },
  ];

  return (
    <header className={`sticky top-0 left-0 right-0 z-50 bg-white transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-50 rounded-xl flex items-center justify-center text-primary-600 group-hover:bg-primary-600 group-hover:text-white transition-all duration-300">
                <FaTooth className="text-xl sm:text-2xl" />
              </div>
              <div className="flex flex-col">
                <span className="text-lg sm:text-2xl font-bold text-gray-900 tracking-tight leading-none">Mandarona</span>
                <span className="text-xs sm:text-sm text-gray-500 font-medium tracking-wide">Centre Dentaire</span>
              </div>
            </Link>
